...
interface Stasiun {
  ...
  prioritas: string;
  keterangan: string | null;
  status_accelerometer: string; // The remapped master status column ('installed' / 'not_installed')
  accelerometer: string | null;  // New Column 1: History Sensor Name
  seismometer: string | null;    // New Column 2: History Sensor Name
  digitizer_komunikasi: string;
  tipe_shelter: string | null;
  lokasi_shelter: string;
  penjaga_shelter: string;
...
const StationMap = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Record<string, boolean>>({
    ...
    jaringan: true, 
    kode_stasiun: true,
    accelerometer: true,        // Active Sensor Name
    seismometer: true,          // Active Sensor Name
    lokasi: true,
    provinsi: true,
    upt_penanggung_jawab: true,
    status: true,
    prioritas: true,
    status_accelerometer: false, // Installation Status string
    digitizer_komunikasi: true,
    tahun_instalasi: true,
    lintang: true,
    bujur: true,
  ...
  const handleImportCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.data.success) {
        setImportSuccess(`Successfully imported ${response.data.insertedCount} stations`);
        const stationsResponse = await axiosServer.get("/api/stasiun/public/sensor"); // ROUTE UPDATED
        setData(stationsResponse.data);
      } else {
        setImportError(response.data.message || 'Import failed');
      }
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      "Net", "Jaringan", "Kode Stasiun", "Accelerometer (Sensor)", "Seismometer (Sensor)", 
      "Lokasi", "Provinsi", "UPT", "Status", "Tahun Instalasi", "Lintang", "Bujur", 
      "Elevasi", "Prioritas", "Keterangan", "Status Accelerometer", "Digitizer Komunikasi",
      "Tipe Shelter", "Lokasi Shelter", "Penjaga Shelter", "Kondisi Shelter",
      "Assets Shelter", "Access Shelter", "Photo Shelter",
      "Penggantian Terakhir Alat", "Updated At"
    ];
    ...
    const csvRows = filteredData.map(station => [
      station.net, station.jaringan, station.kode_stasiun, station.accelerometer || "-", station.seismometer || "-",
      station.lokasi, station.provinsi, station.upt_penanggung_jawab, station.status, station.tahun_instalasi,
      station.lintang, station.bujur, station.elevasi, station.prioritas, station.keterangan || "", 
      station.status_accelerometer, station.digitizer_komunikasi, station.tipe_shelter || "",
      station.lokasi_shelter, station.penjaga_shelter, station.kondisi_shelter || "",
      station.assets_shelter || "", station.access_shelter || "",
      station.photo_shelter || "", station.penggantian_terakhir_alat || "",
      station.updated_at
  ...
  const filteredData = data && Array.isArray(data) ? data.filter((station) => {
    ...
    return (
      ...
      (filters.tahun_instalasi.length === 0 || filters.tahun_instalasi.includes(String(station.tahun_instalasi))) &&
      (filters.status.length === 0 || filters.status.includes(station.status)) &&
      (filters.status_accelerometer.length === 0 || filters.status_accelerometer.includes(station.status_accelerometer)) 
    );
  }) : [];

  const allOptions = useMemo(() => {
    if (!data || !Array.isArray(data)) return { provinsi: [], upt: [], tahun: [], prioritas: [], status: [], status_accelerometer: [] };
    ...
    return {
      ...
      prioritas: Array.from(new Set(data.map(s => s.prioritas))).filter(v => v !== null && v !== undefined).sort(),
      status: Array.from(new Set(data.map(s => s.status))).filter(v => v !== null && v !== undefined).sort(),
      status_accelerometer: ["installed", "not_installed"] 
    };
  }, [data]);
  ...
  // Upgraded: Modified array layout structure to support clean dynamic sorting
  const columns: ColumnDef<Stasiun>[] = [
    {
      header: "Net",
      accessorKey: "net",
    ...
    {
      header: "Seismometer (Sensor)",
      accessorKey: "seismometer",
      enableSorting: true,
      cell: (info) => info.getValue() || "-",
    },
    {
    ...
    {
      header: "Accel Status",
      accessorKey: "status_accelerometer",
      enableSorting: true,
      cell: (info) => info.getValue(),
    },
    {
